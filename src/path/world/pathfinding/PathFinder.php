...
class PathFinder{
	...
	public function createEntityPathTo1($level, $entityFrom, $entityTo, $dist){
		return $this->createEntityPathTo3($level, $entityFrom, $entityTo->x, $entityTo->boundingBox->minY, $entityTo->z, $dist);
	}

	public function createEntityPathTo2($level, $entityIn, $targetPos, $dist){
		return $this->createEntityPathTo3($level, $entityIn, $targetPos->getX() + 0.5, $targetPos->getY() + 0.5, $targetPos->getZ() + 0.5, $dist);
	}

	private function createEntityPathTo3($level, $entityIn, $x, $y, $z, $distance){
		$this->path->clearPath();
		$this->nodeProcessor->initProcessor($level, $entityIn);
